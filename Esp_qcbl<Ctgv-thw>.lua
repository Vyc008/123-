...
-- ==========================================
-- HOOK: BẢO VỆ TỐC ĐỘ KHỎI SPRINT SCRIPT CỦA MAP
-- ==========================================
pcall(function()
    ...
    mt.__newindex = newcclosure(function(t, k, v)
        if not checkcaller() and t:IsA("Humanoid") and k == "WalkSpeed" and getgenv().loopW then
            -- Chặn game tự đổi tốc độ khi đang chạy bộ/chạy nhanh
            return oldNewIndex(t, k, getgenv().Walkspeed)
        end
...
-- ==========================================================
-- LOGIC DI CHUYỂN AN TOÀN CHỐNG ANTI-CHEAT & CHỐNG KẸT (V2.1 - DEEP SCAN FIX MAP)
-- ==========================================================
local moveConn = RunService.Heartbeat:Connect(function()
    ...
    -- ĐÃ FIX: Bỏ qua "cooldown", "cam" để không bị nhận nhầm trong map
    local riskKeywords = {"knock", "down", "carry", "grab", "stun", "ragdoll", "bind", "taze", "freeze", "arrest"}
    local safeKeywords = {"cool", "cam", "mouse", "ui", "frame", "gui"}
    
    local function checkWord(str)
        str = string.lower(str)
        for _, safe in ipairs(safeKeywords) do
            if string.find(str, safe) then return false end
        end
        for _, kw in ipairs(riskKeywords) do
            if string.find(str, kw) then return true end
        end
        return false
    end
    ...
    for _, child in ipairs(char:GetDescendants()) do
        if child:IsA("BoolValue") and child.Value == true then
            if checkWord(child.Name) then isDownedState = true; break end
        elseif child:IsA("StringValue") then
            if checkWord(child.Value) or checkWord(child.Name) then isDownedState = true; break end
        elseif child:IsA("NumberValue") or child:IsA("IntValue") then
            if child.Value > 0 and checkWord(child.Name) then isDownedState = true; break end
        end
    end

    if not isDownedState then
        local function checkObjAttrs(obj)
            for attrName, attrValue in pairs(obj:GetAttributes()) do
                if type(attrValue) == "boolean" and attrValue == true and checkWord(attrName) then return true end
                if type(attrValue) == "string" and (checkWord(attrName) or checkWord(attrValue)) then return true end
                if type(attrValue) == "number" and attrValue > 0 and checkWord(attrName) then return true end
            end
            return false
        end
        if checkObjAttrs(char) or checkObjAttrs(hum) then
            isDownedState = true
        end
    end
